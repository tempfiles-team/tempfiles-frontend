...
export const FileListBox: React.FC<FileListBoxProps> = ({
  ...
  <S.FileListBoxContainer onClick={click}>
    ...
    <div className='long'>
    {isEncrypted ? "🔐" : ""} {fileId != '' ? "ID: "+ fileId+ " / " : ""}파일이름: {filename} / 크기:{size} / 업로드날짜:{uploadDate.year}-
      {uploadDate.month}-{uploadDate.day}
    </div>

    <div className='middle'>
      <p>{isEncrypted ? "🔐" : ""} {fileId != '' ? "ID: "+ fileId+ " / " : ""}파일이름: {filename}</p>
      <p>크기:{size} / 업로드날짜:{uploadDate.year}-
      {uploadDate.month}-{uploadDate.day}</p>
    </div>

    <div className='short'>
      <p>{isEncrypted ? "🔐" : ""} {fileId != '' ? "ID: "+ fileId+ " / " : ""}크기:{size}</p>
      <p>파일이름: {filename}</p>
